...
new class extends Component {
    ...
    public function newCommunicationLogs(): LengthAwarePaginator
    {
        ...
        $previousLoginAt = $admin->previous_login_at;
        ...
        return CustomerCommunicationLog::query()
            ...
            ->when($previousLoginAt instanceof CarbonInterface, function ($query) use ($previousLoginAt): void {
                $query->where('submitted_at', '>', $previousLoginAt);
            })
    ...
    {
        $previousLoginAt = $this->adminUser()->previous_login_at;

        if (! $previousLoginAt instanceof CarbonInterface) {
            return __('All submitted logs');
        }

        return __('Since :date', [
            'date' => $previousLoginAt->copy()->timezone($this->userTimezone())->format('M j, g:i A'),
        ]);
    }
    ...
    {
        return $log->isUpdateRequested() ? 'amber' : 'emerald';
    }
    ...
    {
        $classes = 'grid min-h-16 w-full grid-cols-1 gap-3 rounded-md px-3 py-3 text-left text-sm transition hover:bg-zinc-50 focus:outline-none focus:ring-2 focus:ring-blue-500 dark:hover:bg-white/5 md:grid-cols-[1.2fr_.9fr_1fr_1.1fr_1.7fr] md:items-center';

        if (! $this->logIsRead($log)) {
            $classes .= ' bg-blue-50/70 dark:bg-blue-500/10';
        }

        if ($log->requires_follow_up) {
            $classes .= ' ring-1 ring-amber-200/80 dark:ring-amber-500/30';
        }
    ...
    private function markLogRead(CustomerCommunicationLog $log): void
    {
        ...
        if ($this->logIsReadByAdmin($log)) {
            $log->readByUsers()->updateExistingPivot($admin->id, [
                'read_at' => now(),
            ]);
        ...
        $log->readByUsers()->attach($admin->id, [
            'read_at' => now(),
        ]);
    }
...
<section class="flex h-full w-full flex-1 flex-col gap-6">
    ...
    <div class="grid gap-6 lg:grid-cols-3">
        <section class="space-y-4 lg:col-span-2">
            <div class="flex flex-col gap-1">
                <flux:heading size="lg">{{ __('New Log Entries') }}</flux:heading>
                ...
                >
                    ...
                    <flux:table.rows>
                        @foreach ($this->newCommunicationLogs as $log)
                            <flux:table.row :key="'admin-new-log-'.$log->id" wire:click="openLog('{{ $log->id }}')" class="{{ $this->logRowClass($log) }}">
                                <flux:table.cell>
                                ...
                                <flux:table.cell>
                                    <span class="text-zinc-700 dark:text-zinc-200">{{ $this->submittedAtLabel($log) }}</span>
                                    <span class="truncate text-zinc-700 dark:text-zinc-200">{{ $this->loggedByLabel($log) }}</span>
                                </flux:table.cell>
                                <flux:table.cell>
                                    <span class="flex flex-wrap items-center gap-2">
                                                @if (! $this->logIsRead($log))
                                            <flux:badge size="sm" inset="top bottom" color="blue">{{ __('Unread') }}</flux:badge>
                                        @endif
                                        @if ($log->requires_follow_up)
